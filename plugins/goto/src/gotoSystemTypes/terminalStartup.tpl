...
<table summary="" style="width:100%;">
 <tr>
  <td style='width:50%;' class='right-border'>
   ...
   <LABEL for="gotoLdapServer">{tr}LDAP server{/tr}</LABEL>

   {render acl=$gotoLdapServerACL}
    {if $member_of_ogroup}
      (<input type='checkbox' name='gotoLdap_inherit' {if $gotoLdap_inherit} checked {/if} value="1"
        onClick="document.mainform.submit();" class='center'>&nbsp;{tr}inherit from group{/tr})
    {/if}
   {/render}
  ...
  <td>
   ...
   <table summary="" style="width:100%">
    <tr>
     <td style="width:30%"><LABEL for="gotoBootKernel">{tr}Boot kernel{/tr}</LABEL></td>
     <td>
    ...
    <tr>
     <td><LABEL for="gotoKernelParameters">{tr}Custom options{/tr}</LABEL></td>
     <td>
