...
<table style="width:650px;" summary="{tr}Sudo options{/tr}">
 <tr>
  <td style="width: 140px;"><b>{tr}Option name{/tr}</b></td>
  <td style="width: 20px;"></td>
  <td><b>{tr}Value{/tr}</b></td>
  <td><b>{tr}Options{/tr}</b></td>
 </tr>
{foreach from=$sudoOption item=item key=key}
  {foreach from=$item item=entry key=entry_key} 
   <tr>
    ...
    <td style="width:20px;">
     {if $entry.NEGATE}
      {image path="plugins/sudo/html/images/negate.png"}
...
{render acl=$ACL}   
     {image path="plugins/sudo/html/images/negate.png" action="negOption_{$key}_{$entry_key}"}
