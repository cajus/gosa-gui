 ...
 *
 * ID: $$Id: getxls.php 19301 2010-07-30 16:12:29Z cajus $$
 *
  ...
  if($mode == 2){	// Single Entry Export !
    ...
    switch ($d){
      ...
      case get_people_ou() : 
        ...
        $intitul =  array(tr("Birthday").":", tr("Sex").":", tr("Surname")."/".tr("Given name").":",tr("Language").":");

        // name of the xls file
        $name_section = tr("Users");
        $worksheet    = $workbook->addworksheet(tr("Users"));
        $worksheet->set_column('A:B', 51);

        $user_nbr =  count($user);
        $worksheet->write('A1',sprintf(tr("User list of %s on %s"),$n,$date),$title_title);
        $r=3;
        for($i=1;$i<$user_nbr;$i++)
        {
          if($i>1)
            $worksheet->write('A'.$r++,"");
          $worksheet->write('A'.$r++,tr("User ID").": ".$user[$i][0],$title_bold);
      ...
      case get_groups_ou(): 
        ...
        $intitul   = array(tr("Members").":");

        //name of the xls file
        $name_section=tr("Groups");

        $worksheet = $workbook->addworksheet(tr("Groups"));
        $worksheet->set_column('A:B', 51);

        //count number of groups
        $groups_nbr=count($groups);
        $worksheet->write('A1', sprintf(tr("Groups of %s on %s"), $n, $date),$title_title);
        $r=3;
        for($i=1;$i<$groups_nbr;$i++)
        {
          $worksheet->write('A'.$r++,tr("User ID").": ".$groups[$i][0][0],$title_bold);
          for($j=1;$j<=2;$j++)
          {
            $r++;
            $worksheet->write('A'.$r,$intitul[$j-1]);
     ...
     case get_ou("systemManagement", "systemRDN"): 
       ...
       $name_section=tr("Servers");
       $computers= $ldap->gen_xls($dn,"(&(objectClass=*)(cn=*))",array("cn","description","uid"));

       $intitul=array(tr("Description").":",tr("User ID").":");
       $worksheet = $workbook->addworksheet(tr("Computers"));
       ...
       for($i=1;$i<$computers_nbr;$i++)
       {
         if($i>1)
           $worksheet->write('A'.$r++,"");
         $worksheet->write('A'.$r++,tr("Common name").": ".$computers[$i][0],$title_bold);
         for($j=1;$j<3;$j++)
         {
           $r++;
           $worksheet->write('A'.$r,$intitul[$j-1]);
    ...
    break;
     ...
     case get_ou("servgeneric", "serverRDN"): $servers= $ldap->gen_xls($dn,"(objectClass=*)",array("cn"));
       $intitul=array(tr("Server name").":");

       //name of the xls file
       $name_section=tr("Servers");

       $worksheet = $workbook->addworksheet(tr("Servers"));
       $worksheet->set_column('A:B', 51);

       //count number of servers
       $servers_nbr=count($servers);
       $worksheet->write('A1',sprintf(tr("Servers of %s on %s"), $n, $date),$title_title);
       $r=3;
       $worksheet->write('A'.$r++,tr("Servers").": ",$title_bold);
     ...
     case "dc=addressbook,": //data about addressbook
       ...
       $intitul=  array(tr("Common name").":",tr("Display name").":",tr("Fax").":",
                        tr("Name")."/".tr("Given name").":",tr("Home phone").":",
                        tr("Home postal address").":",tr("Initials").":",tr("Location").":",
                        tr("Mail address").":",tr("Mobile phone").":",tr("City").":",
                        tr("Postal address").":",tr("Pager").":",tr("Phone number").":",
                        tr("Address").":",tr("Postal code").":",tr("Surname").":",
                        tr("State").":",tr("Function").":");

       //name of the xls file
       $name_section=tr("Address book");
       ...
       $worksheet->write('A1',sprintf(tr("Address book of %s on %s"),$n, $date),$title_title);
       $r=3;
       for($i=1;$i<$address_nbr;$i++)
       {
         if($i>1)
           $worksheet->write('A'.$r++,"");
         $worksheet->write('A'.$r++,tr("Common Name").": ".$address[$i][0],$title_bold);
         for($j=1;$j<19;$j++)
         {
           $r++;
           $worksheet->write('A'.$r,$intitul[$j]);
  ...
  elseif($mode == 3){ // Full Export !
    ...
    $address= $ldap->gen_xls("dc=addressbook,".$dn,"(objectClass=*)",
          array("cn","displayName","facsimileTelephoneNumber","givenName","homePhone","homePostalAddress",
                ...
                "postalCode","sn","st","title"));
    $address_intitul=
          array("cn",tr("Display name").":",tr("Fax").":",tr("Surname")."/".tr("Given name").":",
                tr("Phone number").":",tr("Postal address").":",tr("Initials").":",tr("City").":",
                tr("Email address").":",tr("Mobile").":",tr("Organization").":",tr("Organizational unit").":",
                tr("Pager").":",tr("Phone number").":",tr("Postal address").":",tr("Postal Code").":",
                tr("Surname").":",tr("State").":",tr("Title").":");

    //name of the xls file
    $name_section=tr("Full");
    $date=date('dS \of F Y ');
    $fname = tempnam("/tmp", "demo.xls");
    $workbook =  new writeexcel_workbook($fname);
    $worksheet = $workbook->addworksheet(tr("Users"));
    $worksheet2 = $workbook->addworksheet(tr("Groups"));
    $worksheet3 = $workbook->addworksheet(tr("Servers"));
    $worksheet4 =$workbook->addworksheet(tr("Computers"));
    $worksheet5 = $workbook->addworksheet(tr("Address book"));
...
# Create a format for the phone numbers
    ...
    $user_nbr=count($user);
    $worksheet->write('A1',sprintf(tr("User list of %s on %s"), $dn, $date),$title_title);
    $r=3;
    for($i=1;$i<$user_nbr;$i++)
    {
      if($i>1)
    ...
    //count number of groups
    $groups_nbr=count($groups);
    $worksheet2->write('A1',sprintf(tr("Groups of %s on %s"), $dn, $date),$title_title);
    $r=3;
    for($i=1;$i<$groups_nbr;$i++)
    {
      $worksheet2->write('A'.$r++,tr("User ID").": ".$groups[$i][0][0],$title_bold);
      for($j=1;$j<=2;$j++)
      {
        $r++;
    ...
    //count number of servers
    $servers_nbr=count($servers);
    $worksheet3->write('A1',sprintf(tr("Servers of %s on %s"),$dn,$date),$title_title);
    $r=3;
    $worksheet3->write('A'.$r++,tr("Servers").": ",$title_bold);
    for($i=1;$i<$servers_nbr;$i++)
    {
      for($j=0;$j<1;$j++)
      {
    ...
    //count number of computers
    $computers_nbr=count($computers);
    $worksheet4->write('A1',sprintf(tr("Computers of %s on %s"),$dn,$date),$title_title);
    $r=3;
    for($i=1;$i<$computers_nbr;$i++)
    {
      if($i>1)
        $worksheet->write('A'.$r++,"");
      $worksheet4->write('A'.$r++,tr("Common name").": ".$computers[$i][0],$title_bold);
      for($j=1;$j<3;$j++)
      {
        $r++;
    ...
    //count number of entries
    $address_nbr=count($address);
    $worksheet5->write('A1',sprintf(tr("Address book of %s on %s"),$dn, $date),$title_title);
    ...
    for($i=1;$i<$address_nbr;$i++)
    {
      ...
      $worksheet5->write('A'.$r++,tr("Common name").": ".$address[$i][0],$title_bold);
      for($j=1;$j<19;$j++)
      {
        $r++;
...
if(!preg_match("/r/",$acl)){
	msg_dialog::display(tr("Permission error"),tr("You have no permission to do LDAP exports!"),FATAL_ERROR_DIALOG);
  exit();
}
