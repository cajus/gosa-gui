...
{else}
	...
	{/render}
{/if}
 {tr}Proxy account{/tr}</h3>

<table border=0 width="100%" cellpadding=0  summary="{tr}Proxy configuration{/tr}">
 <tr>
  <td colspan=2>
...
{/render}
    {tr}Filter unwanted content (i.e. pornographic or violence related){/tr}
  </td>
 </tr>
 <tr>
  <td width="50%">
...
{/render}
    ...
    <LABEL for="startHour">{tr}Limit proxy access to working time{/tr}</LABEL>
    <br>
    <table style="margin-left:20px;"  summary="{tr}Worktime restrictions{/tr}">
     <tr>
      <td>
...
{render acl=$gosaProxyFlagBACL checkbox=$multiple_support checked=$use_filterB}
	...
	>
{/render}
    <LABEL for="quota_size">{tr}Restrict proxy usage by quota{/tr}</LABEL>
    <br>
    <table style="margin-left:20px;"  summary="{tr}Quota configuration{/tr}">
     <tr>
      <td>
...
{/render}
       ...
       <LABEL for="gosaProxyQuotaPeriod">{tr}per{/tr}</LABEL>
{render acl=$gosaProxyFlagBACL}
