...
<table summary="{tr}Domain component{/tr}" style="width:100%" cellpadding=4>
 <tr>
   <td style="width:50%">
     <h3>{tr}Properties{/tr}</h3>
     <table summary="{tr}Properties{/tr}">
      <tr>
       <td><LABEL for="dc">{tr}Locality name{/tr}</LABEL>{$must}</td>
       <td>
{render acl=$dcACL}
    	<input type='text' id="dc" name="dc" size=25 maxlength=60 value="{$dc}" title="{tr}Name of locality to create{/tr}">
{/render}
       </td>
      </tr>
      <tr>
