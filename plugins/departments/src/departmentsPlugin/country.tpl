...
<table summary="{tr}Country{/tr}" style="width:100%" cellpadding=4>
 <tr>
   <td style="width:50%">
     <h3>{tr}Properties{/tr}</h3>
     <table summary="{tr}Properties{/tr}">
      <tr>
       <td><LABEL for="c">{tr}Country name{/tr}</LABEL>{$must}</td>
       <td>
{render acl=$cACL}
    	<input type='text' id="c" name="c" size=25 maxlength=60 value="{$c}" title="{tr}Name of country to create{/tr}">
{/render}
       </td>
      </tr>
      <tr>
