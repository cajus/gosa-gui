...
<html>
  <head>
    <title>GOsa - {tr}Change your password{/tr}</title>
  ...
  <body>
    ...
    <form action='password.php{$params}' method='post' name='mainform' onSubmit='js_check(this);return true;'>
        ...
        {if $changed}
            ...
            <h3>{tr}Your password has been changed successfully.{/tr}</h3>

        {else}

                <h3>{tr}Password change{/tr}</h3>
                ...
                <p class="infotext">{tr}Enter the current password and the new password (twice) in the fields below and press the 'Set password' button.{/tr}</p>

                <table summary="{tr}Change password{/tr}">
                    {if $show_directory_chooser}
                        <tr>
                            <td>{tr}Directory{/tr}</td>
                            <td>
                                <select name='server'  title='{tr}Directory{/tr}'>
                    ...
                    {/if}
                <tr>
                    <td><b>{tr}User name{/tr}</b></td>
                    <td>
                        {if $display_username}
                            <input type='text' name='uid' maxlength='40' value='{$uid}' 
                                title='{tr}User name{/tr}' onFocus="nextfield= 'current_password';">
                        {else}
                            <i>{$uid}</i>
                        {/if}
                    </td>
                ...
                <tr>
                    <td><b><LABEL for="new_password_repeated">{tr}Repeat new password{/tr}</LABEL></b></td>
                    <td>
            ...
            <div class="plugin-actions">
                <button type='submit' name='apply' 
                    title='{tr}Click here to change your password{/tr}'>{tr}Set password{/tr}</button>
