...
{else}
    ...
    {/if}
			...
			<table summary='{tr}Error message title{/tr}'><tr><td>
		...
		{/if}
			...
			<table summary='{tr}Error message{/tr}' 
		...
		{if $i_Type == $smarty.const.ERROR_DIALOG || $i_Type == $smarty.const.WARNING_DIALOG || $i_Type == $smarty.const.INFO_DIALOG}
						<button type='submit' name='MSG_OK{$i_ID}'>{tr}OK{/tr}</button> 
		{elseif $buttononst.CONFIRM_DIALOG || $i_Type == $smarty.const.OK_CANCEL_DIALOG}
						<button type='submit' name='MSG_OK{$i_ID}'>{tr}OK{/tr}</button>
						<button type='submit' name='MSG_CANCEL{$i_ID}'>{tr}Cancel{/tr}</button>
		{/if}
					</td>
				</tr>
			</table>
    ...
    {/if}
		...
		{if $i_Type == $smarty.const.ERROR_DIALOG || $i_Type == $smarty.const.WARNING_DIALOG || $i_Type == $smarty.const.INFO_DIALOG}
              ...
              onClick='next_msg_dialog();'>{tr}OK{/tr}</button>
		{elseif $i_Type == $smarty.const.CONFIRM_DIALOG}
              ...
              onClick='next_msg_dialog();'>{tr}Cancel{/tr}</button>
