...
$smarty->assign("title", "GOsa - ".tr("Help browser"));
...
if(session::global_is_set('current_class_for_help')){
  ...
  if($helpdir == ""){
    ...
    $smarty->assign("help_contents","<br><h2>".tr("There is no help file specified for this class"))."</h2>";
...
$backwardlink  = "<a href=\"?pg=%s\"  class=\"maintitlebar\">
                  <img src='images/back.png' class=\"center\" align=\"middle\" alt=\"".tr("previous")."\" border=\"0\">
                 </a>";

$forwardlink   = "<a href=\"?pg=%s\"  class=\"maintitlebar\">
                  <img src='images/forward.png' class=\"center\" align=\"middle\" alt=\"".tr("next")."\" border=\"0\">
                 </a>";
...
if(isset($_POST['search'])){
  ...
  $smarty->assign("help_contents","<h2>".sprintf(tr("Help directory '%s' is not accessible, can't read any help files."),$helpdir))."</h2><br>";
  $header= "<!-- headers.tpl-->".$smarty->fetch(get_template_path('headers.tpl'));
  $display= (  $header.$smarty->fetch(get_template_path('help.tpl')));
  echo $display;
}
