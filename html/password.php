...
if (!is_readable(CONFIG_DIR."/".CONFIG_FILE)) {
    msg_dialog::display(
        tr("Fatal error"),
        sprintf(
            tr("GOsa configuration %s/%s is not readable. Aborted."),
...
if (!(is_dir($smarty->compile_dir) && is_writable($smarty->compile_dir))) {
    msg_dialog::display(
        ...
        sprintf(
            tr("Compile directory %s is not accessible!"),
...
if (isset($_GET['method'])) {
    ...
    if (!isset($available[$method])) {
        msg_dialog::display(
            tr("Password method"),
            tr("Error: Password method not available!"),
            FATAL_ERROR_DIALOG
        );
        exit;
    }
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['apply'])) {
    ...
    if(!tests::is_uid($uid)) {
        $message[]= msgPool::invalid(tr("Login"));
    ...
    if(!count($message)){
        ...
        if ($ui === NULL) {
            ...
            if (!preg_match("/w/i", $acls)) {
                $message[]= tr("You have no permissions to change your password!");
            }
        }
    }
...
if ($ssl != "" && $config->get_cfg_value("core","warnSSL") == 'true') {
    $smarty->assign(
        "ssl",
        "<b>".tr("Warning").":</b> ".tr("Session will not be encrypted.").
        " <a style=\"color:red;\" href=\"".htmlentities($ssl)."\"><b>".
        tr("Enter SSL session")."</b></a>!"
    );
} else {
    $smarty->assign("ssl", "");
}
