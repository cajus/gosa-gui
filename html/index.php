...
function displayLogin()
{
    ...
    if (file_exists("$BASE_DIR/ihtml/themes/$theme/blacklist")) {
        ...
        foreach ($blocks as $block) {
            if (preg_match('/'.preg_quote($block).'/', $_SERVER['HTTP_USER_AGENT'])) {
                die(sprintf(tr("Your browser (%s) is blacklisted for the current theme!"), $block));
            }
        }
    }
    ...
    if ($ssl != "" && $config->get_cfg_value("core",'warnSSL') == 'true') {
        $smarty->assign("ssl", sprintf(tr("This session is not encrypted. Click %s to enter an encrypted session."), "<a href=\"$ssl\">".bold(tr("here"))."</a>"));
    } else {
        $smarty->assign("ssl", "");
    }

    if(!$config->check_session_lifetime()) {
        $smarty->assign ("lifetime", tr("The configured session lifetime will be overridden by php.ini settings!"));
    } else {
...
if (!is_readable(CONFIG_DIR."/".CONFIG_FILE)) {
    msg_dialog::display(tr("Configuration error"),sprintf(tr("GOsa configuration %s/%s is not readable. Aborted."), CONFIG_DIR,CONFIG_FILE),FATAL_ERROR_DIALOG);
    exit();
}
...
if (!(is_dir($smarty->compile_dir) && is_writable($smarty->compile_dir))) {
    msg_dialog::display(tr("Smarty error"),sprintf(tr("Compile directory %s is not accessible!"),
...
$smarty->assign ("cookies", tr("Your browser has cookies disabled: please enable cookies and reload this page before logging in!"));
...
if ($config->get_cfg_value("core","htaccessAuthentication") == "true" ) {
    if (!isset($_SERVER['REMOTE_USER'])) {
        msg_dialog::display(tr("Configuration error"), tr("Broken HTTP authentication setup!"), FATAL_ERROR_DIALOG);
        exit;
    }
    ...
    $server= $tmp['server'];
    if ($username == "") {
        msg_dialog::display(tr("Error"), tr("Cannot find a valid user for the current HTTP authentication!"), FATAL_ERROR_DIALOG);
        exit;
    }
    if ($server == "") {
        msg_dialog::display(tr("Error"), tr("Cannot find a unique user for the current HTTP authentication!"), FATAL_ERROR_DIALOG);
        exit;
    }
...
if (($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['login'])) || $htaccess_authenticated) {
    ...
    if (!$htaccess_authenticated) {
        ...
        if (!preg_match("/^[@A-Za-z0-9_.-]+$/", $username)) {
            $message= tr("Please specify a valid user name!");
            $ok= false;
        } elseif (mb_strlen(get_post("password"), 'UTF-8') == 0) {
            $message= tr("Please specify your password!");
    ...
    if ($ok) {
        ...
        if ($htaccess_authenticated) {
            ...
            if ($ui === NULL || !$ui) {
                msg_dialog::display(tr("Authentication error"), tr("Cannot retrieve user information for HTTP authentication!"), FATAL_ERROR_DIALOG);
                exit;
            }
        } else {
        ...
        if ($ui === NULL || !$ui) {
            $message= tr("Please check the username/password combination!");
            $smarty->assign ('nextfield', 'password');
      ...
      config to session. */
            ...
            if ($config->boolValueIsTrue("core","handleExpiredAccounts")) {
                ...
                if ($expired == POSIX_ACCOUNT_EXPIRED) {
                    $message= tr("Account locked. Please contact your system administrator!");
                    $smarty->assign ('nextfield', 'password');
