...
// Bind plugin locale dirs
$path = preg_replace("/html[\/]*$/","plugins", getcwd());
$dir = opendir($path);
while($file = readdir($dir)){
    if(!preg_match("/^\./", $file) && file_exists("../plugins/{$file}/plugin.dsc")){
        $lpath = "{$path}/{$file}/locale";
        bindtextdomain($file, $lpath);
    }
}
...
if (isset($global_check) && $config->boolValueIsTrue("core","forceGlobals")){
  msg_dialog::display(
            tr("PHP configuration"),
            tr("Fatal error: Register globals is active. Please fix this in order to continue."),
            FATAL_ERROR_DIALOG);
...
if ($config->boolValueIsTrue("core","handleExpiredAccounts")){
    ...
    if ($expired == POSIX_WARN_ABOUT_EXPIRATION && !session::is_set('POSIX_WARN_ABOUT_EXPIRATION__DONE')){
        ...
        msg_dialog::display(tr("Password change"), tr("Your password is about to expire, please change your password!"), INFO_DIALOG);
...
if (isset($_GET['plug']) && $plist->plugin_access_allowed($_GET['plug'])){
  ...
  if ($plugin_dir == ""){
    ...
    $plugin_dir= "$BASE_DIR/plugins/welcome/src/welcomePlugin";
}
...
if (function_exists("memory_get_usage")){
  if (memory_get_usage() > (to_byte(ini_get('memory_limit')) - 2048000 )){
    msg_dialog::display(tr("Configuration error"), tr("Running out of memory!"), WARNING_DIALOG);
  }
}
...
$loggedin = sprintf(tr("You're logged in as %s"), 
    "<span>".$ui->cn." [".$ui->username."] / ".$config->current['NAME']."</span> &nbsp;");
if($ui->ignore_acl_for_current_user()){
    $loggedin = "<font color='red'>".tr("ACLs are disabled")."</font>&nbsp;".$loggedin;
}
...
if (is_file("$plugin_dir/main.inc")){
  ...
  msg_dialog::display(
      tr("Plug-in"),
      sprintf(tr("Fatal error: Cannot find any plugin definitions for plugin %s!"), bold($plug)),
      FATAL_ERROR_DIALOG);
  exit();
}
...
if (isset($_POST) && count($_POST)){
  if (!isset($_POST['php_c_check'])){
    msg_dialog::display(
            ...
            sprintf(tr("Fatal error: not all POST variables have been transfered by PHP - please inform your administrator!")),
            FATAL_ERROR_DIALOG);
    exit();
  }
}
